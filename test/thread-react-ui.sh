...
# Each identity gets its OWN terminal namespace. This fixture used to run dale
# and ernie from a single terminal with STITCHPAD_STEAL=1, so whichever joined
# last held the claim and the other's `say` was refused by the terminal-identity
# guard ("this terminal is claimed by @ernie, but you are @dale") — the guard
# that stops a ghost-poster writing as someone else. Two agents means two
# terminals; the assertions below are unchanged.
...
cd "$tmp"
"$SP" init >/dev/null
STITCHPAD_NAME=dale STITCHPAD_TERMINAL_NAMESPACE=tr-dale "$SP" join dale claude >/dev/null 2>&1 || true
STITCHPAD_NAME=ernie STITCHPAD_TERMINAL_NAMESPACE=tr-ernie "$SP" join ernie codex >/dev/null 2>&1 || true
PAD="$tmp/.stitchpad/stitchpad.md"

# ── 1. say mints an id and prints it ─────────────────────────────
out="$(STITCHPAD_NAME=dale STITCHPAD_TERMINAL_NAMESPACE=tr-dale "$SP" say "first message" 2>&1)"
...
out="$(STITCHPAD_NAME=ernie STITCHPAD_TERMINAL_NAMESPACE=tr-ernie "$SP" say --re "$id" "threaded answer" 2>&1)"
rid="$(printf '%s' "$out" | grep -o 'm-[a-z0-9]*' | head -1)"
grep -q "^## @ernie · .* · #$rid · re:#$id\$" "$PAD" || fail "reply header must carry re:#$id"
if STITCHPAD_NAME=ernie STITCHPAD_TERMINAL_NAMESPACE=tr-ernie "$SP" say --re "m-nope99" "ghost" >/dev/null 2>&1; then
  fail "reply to a nonexistent id must refuse"
fi

# ── 3. react appends a system line; same emoji toggles it off ────
STITCHPAD_NAME=ernie STITCHPAD_TERMINAL_NAMESPACE=tr-ernie "$SP" react "$id" "👍" >/dev/null
...
if STITCHPAD_NAME=ernie STITCHPAD_TERMINAL_NAMESPACE=tr-ernie "$SP" react "m-nope99" "👍" >/dev/null 2>&1; then
  fail "react to a nonexistent id must refuse"
fi

# ── 4. @flock expands like @all (gang-prompt) ────────────────────
STITCHPAD_NAME=dale STITCHPAD_TERMINAL_NAMESPACE=tr-dale "$SP" say "@flock stand-up in 5" >/dev/null
...
```'
STITCHPAD_NAME=dale STITCHPAD_TERMINAL_NAMESPACE=tr-dale "$SP" say "$body" >/dev/null
...
out="$(STITCHPAD_NAME=dale STITCHPAD_TERMINAL_NAMESPACE=tr-dale "$SP" say "draft: v1 of the plan" 2>&1)"
aid="$(printf '%s' "$out" | grep -o 'm-[a-z0-9]*' | head -1)"
STITCHPAD_NAME=ernie STITCHPAD_TERMINAL_NAMESPACE=tr-ernie "$SP" say --re "$aid" "threaded on the draft" >/dev/null
STITCHPAD_NAME=dale STITCHPAD_TERMINAL_NAMESPACE=tr-dale "$SP" amend "$aid" "final: v2 of the plan" >/dev/null
...
if STITCHPAD_NAME=ernie STITCHPAD_TERMINAL_NAMESPACE=tr-ernie "$SP" amend "$aid" "hijacked" >/dev/null 2>&1; then
  fail "amending someone else's message must refuse"
fi
if STITCHPAD_NAME=dale STITCHPAD_TERMINAL_NAMESPACE=tr-dale "$SP" amend "m-nope99" "ghost" >/dev/null 2>&1; then
  fail "amending a nonexistent id must refuse"
fi

# ── 7. run: steps become a live-amending timeline; failure threads ─
STITCHPAD_NAME=dale STITCHPAD_TERMINAL_NAMESPACE=tr-dale "$SP" run --title "contract check" "ok step"="true" "quick math"="test 2 -eq 2" >/dev/null 2>&1 \
...
if STITCHPAD_NAME=dale STITCHPAD_TERMINAL_NAMESPACE=tr-dale "$SP" run --title "fail check" "boom"="exit 3" >/dev/null 2>&1; then
